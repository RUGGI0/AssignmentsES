...
// --- Tx and Rx buffers --- //
#define SIZETX 128 // Tx buffer size -> handles periodic logs plus button-triggered bursts
// - highest amount of char composing a MBATT message is 13
// - highest amount of char composing a MDIST message is 12
// - highest amount of char composing a MANGLE message is 23
// - highest amount of char composing a MBUF message (triggered by RE9) is 13
// this gives margin when messages are queued close together

#define SIZERX 64 // Rx buffer size -> at 9600 baud it tolerates temporary task jitter
...
#define MSG_PAYLOAD_SIZE (12)

typedef struct {
	int state;
	char msg_type[MSG_TYPE_SIZE]; // message type is up to 5 chars + string terminator
	char msg_payload[MSG_PAYLOAD_SIZE];  // payload plus string terminator
	int index_type;
	int index_payload;
} parser_state;
...
#define AVOIDANCE_STEP_1 (4) // turning 90 degrees clockwise
#define AVOIDANCE_STEP_2 (5) // moving forward for 2 seconds
#define AVOIDANCE_STEP_3 (6) // turning 90 degrees anti-clockwise (back to previous heading)
...
#define OBSTACLE_THRESHOLD_CM (35)
#define MAX_AVOIDANCE_ATTEMPTS (3)
...
typedef struct{
    ...
    int acc_filtered_values[3]; // filtered accelerometer x,y,z values
    int imu_filter_ready;
    float gyro_yaw;
